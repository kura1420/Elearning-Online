...
<script>
var obj = @php echo json_encode($tahunAjaranJadwals); @endphp;
</script>
<script src="{{ asset('assets/js/sekolah/tahun_ajaran/edit.js') }}"></script>
@endsection
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            ...
                                            <select name="periode_awal" id="periode_awal" class="form-control">
                                                ...
                                                <option value="{{ $i }}" {{ $row->periode_awal == $i ? 'selected' : '' }}> {{ $i }} </option>
                                                @endfor
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            ...
                                            <select name="periode_akhir" id="periode_akhir" class="form-control">
                                                <option value="">- Pilih -</option>
                                                @for ($i=date('Y') - 5; $i<date('Y') + 5; $i++)
                                                <option value="{{ $i }}" {{ $row->periode_akhir == $i ? 'selected' : '' }}> {{ $i }} </option>
                                                @endfor
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        @foreach ($kelas as $key => $kls)
                                            <div class="tab-pane text-left fade show {{ $key == 0 ? 'active' : '' }}" id="vert-tabs-{{ strtolower(str_replace(' ', '-', $kls->nama)) }}" role="tabpanel" aria-labelledby="vert-tabs-{{ strtolower(str_replace(' ', '-', $kls->nama)) }}-tab">
                                                ...
                                                <table class="table table-bordered">
                                                    ...
                                                    @foreach ($tahunAjaranJadwals as $tahunAjaranJadwal)
                                                        @if ($tahunAjaranJadwal['kelas_id'] == $kls->id)
                                                        <tr id="{{ $tahunAjaranJadwal['kelas_id'] . '-' . $tahunAjaranJadwal['pelajaran_id'] }}">
                                                            <td> {{ $tahunAjaranJadwal['pelajaran_nama'] }} </td>
                                                            <td> {{ $tahunAjaranJadwal['user_guru_nama'] }} </td>
                                                            <td>
                                                                <button type="button" class="btn btn-sm btn-danger btnDelete" id="{{ $tahunAjaranJadwal['pelajaran_id'] }}" value="{{ $tahunAjaranJadwal['kelas_id'] }}">
